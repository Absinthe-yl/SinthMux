...
function Login({ onLogin, githubEnabled }: { onLogin: () => void; githubEnabled: boolean }) {
  ...
  return <main className="login-shell"><div className="login-card"><span className="login-mark">S/</span><h1>登录 SinthMux</h1>{githubEnabled && <><a className="github-login" href="/api/v1/auth/github/start">使用 GitHub 登录</a><div className="login-divider">或使用用户登录令牌</div></>}<form onSubmit={(event) => void submit(event)}><label className="sr-only" htmlFor="login-token">用户登录令牌</label><input id="login-token" type="password" autoComplete="off" placeholder="粘贴用户登录令牌" value={token} onChange={(event) => setToken(event.target.value)} required /><button type="submit" disabled={busy}>{busy ? "登录中…" : "登录"}</button></form>{error && <div className="notice error">{error}</div>}</div></main>;
}
