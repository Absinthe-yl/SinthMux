...
export SINTHMUX_AGENT_HUB_URL=ws://127.0.0.1:8090/ws/v1/agents/connect

umask 077
mkdir -p .run/bin .run/log

printf '构建 Hub 和 Agent…\n'
go build -o .run/bin/sinthmux-hub ./apps/hub
go build -o .run/bin/sinthmux-agent ./apps/agent
...
cleanup() {
  trap - EXIT INT TERM
  for pid in "$web_pid" "$agent_pid" "$hub_pid"; do
...
.run/bin/sinthmux-agent > .run/log/agent.log 2>&1 &
agent_pid=$!
...
while true; do
  for pid in "$hub_pid" "$agent_pid" "$web_pid"; do
